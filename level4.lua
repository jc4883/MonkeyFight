...
local opponent
	...
	local opponentUsedDJ 
	local opponentIsAirborne
local player
	local usedDJ
	local inLag
...
local function gameOver()
	if (winner == 2) then --monkey gets caught
		physics.pause()
		...
		composer.setVariable("lastFoughtOpponent", 4) 
		composer.gotoScene("tryAgain", { time = 800, effect = "crossFade"})
	elseif( winner == 3) then	--monkey dies by himself
		physics.pause()
...
local function playerLostStock()
	if (playerLives == 0) then
		...
		playerLivesText.text = "P1: 1 unremarkable"
	end
	restorePlayer()
end
...
local function onCollision( event )
	...
	if ( ( obj1.myName == "opponent" and obj2.myName == "ground" ) or
		 ( obj1.myName == "ground" and obj2.myName == "opponent" ) )
	then
    	-- Foot sensor has overlapped a ground object
		if ( event.phase == "began" ) then
			if (obj1.myName == "opponent") then
				obj1.sensorOverlaps = obj1.sensorOverlaps + 1
	...
	then
		...
		if (opponent.x > player.x) then
			opponent:applyLinearImpulse(20 + .3 * opponentPercent, -10)
		else
			opponent:applyLinearImpulse(-20 + .3 * opponentPercent, -10 )
		end
		opponentPercentText.text = opponentPercent .. "%"
...
local function opponentJump()
	if (opponentInStun == true) then
		return
	end
    if (opponentUsedDJ == false) then
		-- Jump procedure here
		opponentUsedDJ = true
		local vx, vy = opponent:getLinearVelocity()
		opponent:setLinearVelocity( vx, 0 )
		opponent:applyLinearImpulse( nil, -7, opponent.x, opponent.y )  
	end
end
...
local function jump( event )
    ...
    if ( event.phase == "began" and player.sensorOverlaps == 1 ) then
		...
		player:applyLinearImpulse( nil, -7, player.x, player.y )  
	end
...
local function opponentStart()
	opponentLoopTimer = timer.performWithDelay( 200, function()
		if (opponent.x > player.x) then
			opponent:setLinearVelocity(-50, nil)
		else 
			opponent:setLinearVelocity(50, nil)
		end
		if (opponentIsAirborne == true) then
			opponentJump()
		end
	end, 0 )
