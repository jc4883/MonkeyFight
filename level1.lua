...
local function gameOver()
	if (winner == 2) then
		...
		timer.performWithDelay(2000, function()
			composer.gotoScene("cutscenes.between1and2.1", { time = 2000, effect = "crossFade"})
		end
		)
	end
end
...
local function onCollision( event )
	...
	then
		...
		opponent:applyLinearImpulse( 30+ .2 * opponentPercent, -8 + .1 * opponentPercent, opponent.x, opponent.y)
	...
	then
		...
		print(opponentPercent)
		opponent:applyLinearImpulse( -30 - .23 * opponentPercent, -8 + .1 * opponentPercent, opponent.x, opponent.y)
		opponentPercentText.text = opponentPercent .. "%"
		opponentInStun = true 
		timer.performWithDelay(217, function()
			opponentInStun = false
	...
	then
		opponentPercent = opponentPercent + 3
		print(opponentPercent)
		opponent:applyLinearImpulse( 0, 90 -.23 * opponentPercent, opponent.x, opponent.y)
		opponentPercentText.text = opponentPercent .. "%"
		opponentInStun = true 
		timer.performWithDelay(217, function()
			opponentInStun = false
	...
	then
		opponentPercent = opponentPercent + 3
		print(opponentPercent)
		opponent:applyLinearImpulse( 0, -40 - .33 * opponentPercent, opponent.x, opponent.y)
		opponentPercentText.text = opponentPercent .. "%"
		opponentInStun = true 
		timer.performWithDelay(217, function()
			opponentInStun = false
...
local function jump( event )
    ...
    if ( event.phase == "began" and player.sensorOverlaps == 1 ) then
		...
		player:applyLinearImpulse( nil, -90, player.x, player.y )  
	end
