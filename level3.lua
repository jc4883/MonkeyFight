...
local function gameOver()
	if (winner == 2) then
		...
		composer.setVariable("lastFoughtOpponent", 3) 
...
local function onCollision( event )
	...
	then
		...
		if (opponent.x > player.x) then
			player:applyLinearImpulse(-30, -30)
			transition.to(opponent, {time=1000, x=410})
			playerInStun = true
			...
			player:applyLinearImpulse(30, -30)
			transition.to(opponent, {time=1000, x=60})
			timer.performWithDelay(1000, function()
				playerInStun = false
			end)
		end
...
local function createControls()
	...
	jumpScreen:addEventListener( "touch", jump )
...
local function opponentStart()
	opponentLoopTimer = timer.performWithDelay( 500, function()
		if (player.x < 70 or player.x > 410) then
			return
		end
		if (opponent.x > player.x) then
			opponent:setLinearVelocity(-10, nil)
		else 
			opponent:setLinearVelocity(10, nil)
		end
	end, 0 )
...
local function newPlayer()
	...
	physics.addBody(player, "dynamic", 
		...
		{ shape = bodyParts["feet"], bounce = 1.0},
