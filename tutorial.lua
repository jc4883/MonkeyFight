...
local function createBackground()
	gameBackground = display.newImageRect(backGroup, "Images.xcassets/gameBackground.png", 570, 360)
...
local function gameOver()
	if (winner == 2) then
		...
		composer.setVariable("lastFoughtOpponent", 1) 
		composer.gotoScene("tryAgain", { time = 800, effect = "crossFade"})
	else
		local winnerScreen = display.newText( uiGroup,
		"Another Day Another Kill...", display.contentCenterX, 100, native.systemFont, 37)
		physics.pause()
...
local function jump( event )
    ...
    if ( event.phase == "began" and player.sensorOverlaps == 1 ) then
		...
		player:applyLinearImpulse( nil, -9, player.x, player.y )  
	end
...
local function createControls()
	...
	jumpText = display.newText("tap to jump",
		jumpScreen.x-display.contentWidth/4 , display.contentCenterY/2, native.systemFont, 30)
	...
	moveText = display.newText("swipe to move",
		display.contentCenterX/2 , display.contentCenterY, native.systemFont, 30)
		moveText:setFillColor(0,0,0)
	...
	attackText = display.newText("swipe/tap to attack",
		display.contentWidth * 3/4 , display.contentHeight*3/4, native.systemFont, 30)
		attackText:setFillColor(0,0,0)

end

local function opponentStart()
	opponentLoopTimer = timer.performWithDelay( 500, function()
		if (opponent.x > player.x) then
			opponent:setLinearVelocity(-100, nil)
		else 
			opponent:setLinearVelocity(100, nil)
		end
	end, 0 )
...
local function newOpponent()
	opponent = display.newRect(display.contentCenterX + 100, display.contentCenterY, 30, 45)
	opponent:setFillColor(.4, .9, .2)
	local opponentBodyParts = { 
		body = {-15,-15,  15,-15,  15,30, -15,30}, --2
		feet = {-13,27, 13,27, 13,31, -13,31}, --3
		head = {-15,-12, -15,-16, 15,-16, 15,-12} --4
	}
	physics.addBody(opponent, "dynamic", 
		{density = 7, bounce = .3}, 
		{ shape = opponentBodyParts["body"]},
		{ shape = opponentBodyParts["feet"], bounce = 1}, 
...
local function newPlayer()
	...
	player = display.newRect( display.contentCenterX - 100, display.contentCenterY, 30, 45 )
...
function scene:create( event )
	...
	playerLivesText = display.newText( uiGroup, "P1: " .. playerLives, -100, display.contentHeight - 40, native.systemFont, 12 )
	...
	exitButton:addEventListener("tap", gotoGame)
...
function scene:hide( event )
	...
	elseif ( phase == "did" ) then
		...
		display.remove(jumpText)
		display.remove(attackText)
		display.remove(moveText)
		display.remove(gameBackground)
		physics.pause()
